...
async def create_account(
    ...
    return AccountInfo(
        account=AccountOut.model_validate(new_account),
...
async def deposit_funds(
    ...
    account = await __get_account(account_id, account_repo)
    __check_authorization(account.user_id, current_user.id)
    account = await account_repo.update_funds(
        account_id, amount_funds.balance_investable_funds
    )
    return AccountInfo(
        account=AccountOut.model_validate(account),
        detail=f"Added funds in the amount of {amount_funds.balance_investable_funds} {account.currency} to the account",
    )
...
async def withdraw_funds(
    ...
    return AccountInfo(
        account=AccountOut.model_validate(account),
        detail=f"Withdrawn funds in the amount of {amount_funds.balance_investable_funds} {account.currency} from the account",
    )
